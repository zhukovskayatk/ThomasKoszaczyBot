...
def premium_status_text(is_active: bool, premium_until, price_month_stars: int, price_year_stars: int) -> str:
    ...
    if is_active:
        return (
            ...
            "Открыто: партнёрский режим 👥, безлимитный ИИ-разбор дат и "
            "приоритетов, неограниченный список дел и привычек, утренний "
            "и вечерний дайджест чек-листа.\n\n"
    ...
    return (
        ...
        "🤖 Безлимитный ИИ-разбор дат/приоритета из текста и голоса "
        "(на бесплатном — 5 попыток на пробу)\n"
        "📋 Список дел и привычек без ограничений (бесплатно — до 7 дел и "
        "3 привычек)\n"
        "☀️ Утренний и вечерний дайджест чек-листа дня\n"
...
def free_ai_limit_reached_text() -> str:
    """
    Бесплатный лимит ИИ-разбора текста/голоса исчерпан (см.
    database.requests.FREE_AI_PARSES_LIMIT) — показывается ОДНОРАЗОВОЙ
    отдельной репликой сразу под обычным подтверждением создания задачи
    (см. handlers/tasks.py::_create_task_with_ai): задача в любом случае
    уже записана, просто без автоматического дедлайна/приоритета — их
    придётся выставить вручную, как было до появления ИИ-разбора.
    """
    return (
        "🤖 Бесплатные попытки умного разбора закончились — дальше ИИ сам "
        "угадывает дату, приоритет и общие дела только с 💎 Premium.\n"
        "Задачу я всё равно записала — просто дедлайн и приоритет придётся "
        "выставить вручную."
    )


def free_task_limit_reached_text() -> str:
    """Бесплатный лимит активных задач исчерпан (см.
    database.requests.FREE_ACTIVE_TASKS_LIMIT) — новая задача НЕ создаётся,
    в отличие от free_ai_limit_reached_text."""
    return (
        "🙈 На бесплатном тарифе можно вести до 7 активных дел одновременно "
        "— сейчас как раз столько. Закрой что-то уже сделанное или оформи "
        "💎 Premium, чтобы снять ограничение совсем."
    )


def free_voice_limit_reached_text() -> str:
    """
    Бесплатный лимит ИИ (общий на текст и голос, см.
    database.requests.FREE_AI_PARSES_LIMIT) уже исчерпан, и это ГОЛОСОВОЕ
    сообщение — в отличие от free_ai_limit_reached_text, тут задачу
    создать НЕЧЕМ (само распознавание речи и есть дорогая часть, без него
    не получить текст), поэтому просто просим написать словами.
    """
    return (
        "🎙 Бесплатные попытки ИИ (текст и голос вместе) закончились — "
        "голосовой ввод дальше только с 💎 Premium.\n"
        "Пока просто напиши, что нужно сделать, текстом — так работает "
        "всегда, без ограничений."
    )


def voice_transcription_unavailable_text() -> str:
    """Голосовое сообщение пришло, но распознать его не получилось (нет
    ключа OpenAI, нет сети, Whisper не смог разобрать) — просим написать
    текстом вместо того, чтобы просто промолчать."""
    return (
        "🎙 Не получилось разобрать голосовое сообщение — попробуй, "
        "пожалуйста, написать текстом."
    )


def free_habit_limit_reached_text() -> str:
    """Бесплатный лимит привычек исчерпан (см.
    database.requests.FREE_HABITS_LIMIT) — новая привычка НЕ создаётся."""
    return (
        "🙈 На бесплатном тарифе можно завести до 3 привычек — сейчас как "
        "раз столько. Оформи 💎 Premium, чтобы добавлять сколько угодно и "
        "получить утренний/вечерний дайджест чек-листа."
    )
...
def welcome_text() -> str:
    ...
    return (
        ...
        "<i>Просто напиши или надиктуй голосом, что нужно сделать, или "
        "воспользуйся меню внизу 👇</i>"
    )
